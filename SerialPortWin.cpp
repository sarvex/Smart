...
void SerialPort::platformSpecificInit()
{
...
Standard destructor.
*/
void SerialPort::platformSpecificDestruct()
{
    CloseHandle(overlap.hEvent);
    delete bytesToWriteLock;
}

QString SerialPort::fullPortNameWin(const QString & name)
{
    QRegExp rx("^COM(\\d+)");
...
to open a device by name, see SerialPort::open(const char*).  This function has no effect
...
bool SerialPort::open(OpenMode mode)
{
    ...
    if (queryMode() == SerialPort::EventDriven)
...
multithreading situations, use SerialPort::bytesAvailable() instead.
*/
...
/*!
Translates a system-specific error code to a SerialPort error code.  Used internally.
*/
...
void SerialPort::ungetChar(char c)
{
    ...
    TTY_WARNING("SerialPort: ungetChar() called on an unbuffered sequential device - operation is meaningless");
}
    ...
    if (isOpen())
    {
        ...
        switch (parity)
        {
            ...
            case PAR_SPACE:
                ...
                {
                    TTY_PORTABILITY_WARNING("SerialPort Portability Warning: Space parity with 8 data bits is not supported by POSIX systems.");
                }
            ...
            case PAR_MARK:
                TTY_PORTABILITY_WARNING("SerialPort Portability Warning:  Mark parity is not supported by POSIX systems");
                Win_CommConfig.dcb.fParity=TRUE;
                break;
    ...
    if (isOpen())
    {
        switch (dataBits)
        {
            ...
            case DATA_5:
                ...
                {
                    TTY_WARNING("SerialPort: 5 Data bits cannot be used with 2 stop bits.");
                }
                else
                {
            ...
            case DATA_6:
                ...
                {
                    TTY_WARNING("SerialPort: 6 Data bits cannot be used with 1.5 stop bits.");
                }
                else
                {
            ...
            case DATA_7:
                if (Settings.StopBits==STOP_1_5)
                {
                    TTY_WARNING("SerialPort: 7 Data bits cannot be used with 1.5 stop bits.");
                }
                else
                {
            ...
            case DATA_8:
                if (Settings.StopBits==STOP_1_5)
                {
                    TTY_WARNING("SerialPort: 8 Data bits cannot be used with 1.5 stop bits.");
                }
                else
                {
    ...
    if (isOpen())
    {
        switch (stopBits)
        {
            ...
            case STOP_1:
                ...
                /*1.5 stop bits*/
            case STOP_1_5:
                TTY_PORTABILITY_WARNING("SerialPort Portability Warning: 1.5 stop bit operation is not supported by POSIX.");
                if (Settings.DataBits!=DATA_5)
                {
                    TTY_WARNING("SerialPort: 1.5 stop bits can only be used with 5 data bits");
                }
                else
                {
            ...
            case STOP_2:
                if (Settings.DataBits==DATA_5)
                {
                    TTY_WARNING("SerialPort: 2 stop bits cannot be used with 5 data bits");
                }
                else
                {
...
void SerialPort::setBaudRate(BaudRateType baudRate)
{
    ...
    if (isOpen())
    {
        switch (baudRate)
        {
            ...
            /*50 baud*/
            case BAUD50:
                TTY_WARNING("SerialPort: Windows does not support 50 baud operation.  Switching to 110 baud.");
                Win_CommConfig.dcb.BaudRate=CBR_110;
                break;

                /*75 baud*/
            case BAUD75:
                TTY_WARNING("SerialPort: Windows does not support 75 baud operation.  Switching to 110 baud.");
                Win_CommConfig.dcb.BaudRate=CBR_110;
                break;
            ...
            case BAUD110:
                ...
                /*134.5 baud*/
            case BAUD134:
                TTY_WARNING("SerialPort: Windows does not support 134.5 baud operation.  Switching to 110 baud.");
                Win_CommConfig.dcb.BaudRate=CBR_110;
                break;

                /*150 baud*/
            case BAUD150:
                TTY_WARNING("SerialPort: Windows does not support 150 baud operation.  Switching to 110 baud.");
                Win_CommConfig.dcb.BaudRate=CBR_110;
                break;

                /*200 baud*/
            case BAUD200:
                TTY_WARNING("SerialPort: Windows does not support 200 baud operation.  Switching to 110 baud.");
                Win_CommConfig.dcb.BaudRate=CBR_110;
                break;
            ...
            case BAUD1200:
                ...
                /*1800 baud*/
            case BAUD1800:
                TTY_WARNING("SerialPort: Windows does not support 1800 baud operation.  Switching to 1200 baud.");
                Win_CommConfig.dcb.BaudRate=CBR_1200;
                break;
            ...
            case BAUD9600:
                ...
                /*14400 baud*/
            case BAUD14400:
                TTY_PORTABILITY_WARNING("SerialPort Portability Warning: POSIX does not support 14400 baud operation.");
            ...
            case BAUD38400:
                ...
                /*56000 baud*/
            case BAUD56000:
                TTY_PORTABILITY_WARNING("SerialPort Portability Warning: POSIX does not support 56000 baud operation.");
            ...
            case BAUD57600:
                ...
                /*76800 baud*/
            case BAUD76800:
                TTY_WARNING("SerialPort: Windows does not support 76800 baud operation.  Switching to 57600 baud.");
                Win_CommConfig.dcb.BaudRate=CBR_57600;
                break;
            ...
            case BAUD115200:
                ...
                /*128000 baud*/
            case BAUD128000:
                TTY_PORTABILITY_WARNING("SerialPort Portability Warning: POSIX does not support 128000 baud operation.");
                Win_CommConfig.dcb.BaudRate=CBR_128000;
                break;

                /*256000 baud*/
            case BAUD256000:
                TTY_PORTABILITY_WARNING("SerialPort Portability Warning: POSIX does not support 256000 baud operation.");
...
*/
void SerialPort::onWinEvent(HANDLE h)
{
    QMutexLocker lock(mutex);
    if (h == overlap.hEvent)
    {
