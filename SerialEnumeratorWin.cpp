...
SerialEnumerator::SerialEnumerator( )
{
    if ( !QMetaType::isRegistered( QMetaType::type("QextPortInfo") ) )
        qRegisterMetaType<PortInformation>("QextPortInfo");
...
//static
QString SerialEnumerator::getRegKeyValue(HKEY key, LPCTSTR property)
{
    DWORD size = 0;
    DWORD type;
...
//static
QString SerialEnumerator::getDeviceProperty(HDEVINFO devInfo, PSP_DEVINFO_DATA devData, DWORD property)
{
    DWORD buffSize = 0;
...
QList<PortInformation> SerialEnumerator::getPorts()
{
    QList<PortInformation> ports;
    enumerateDevicesWin(GUID_DEVCLASS_PORTS, &ports);
    return (ports);
}

void SerialEnumerator::enumerateDevicesWin( const GUID & guid, QList<PortInformation>* infoList )
{
    HDEVINFO devInfo;
...
#ifdef QT_GUI_LIB
bool SerialRegistrationWidget::winEvent( MSG* message, long* result )
{
...
void SerialEnumerator::setUpNotifications( )
{
#ifdef QT_GUI_LIB
    if (notificationWidget)
        return;
    notificationWidget = new SerialRegistrationWidget(this);
    ...
    foreach( PortInformation port, getPorts() )
...
LRESULT SerialEnumerator::onDeviceChangeWin( WPARAM wParam, LPARAM lParam )
{
...
bool SerialEnumerator::matchAndDispatchChangedDevice(const QString & deviceID, const GUID & guid, WPARAM wParam)
{
    bool rv = false;
...
bool SerialEnumerator::getDeviceDetailsWin( PortInformation* portInfo, HDEVINFO devInfo, PSP_DEVINFO_DATA devData, WPARAM wParam )
{
    ...
    portInfo->portName = SerialPort::fullPortNameWin( getRegKeyValue(devKey, TEXT("PortName")) );
