...
functionality for SerialPort.

\section Usage
SerialPort offers both a polling and event driven API.  Event driven is typically easier
to use, since you never have to worry about checking for new data.

\b Example
\code
SerialPort* port = new SerialPort("COM1", SerialPort::EventDriven);
...
public:
    ...
    SerialPort(QueryMode mode = EventDriven);
    SerialPort(const QString & name, QueryMode mode = EventDriven);
    SerialPort(PortSettings const& s, QueryMode mode = EventDriven);
    SerialPort(const QString & name, PortSettings const& s, QueryMode mode = EventDriven);
    ~SerialPort();
    ...
    /*!
         ...
         * To determine when operation is finished, SerialPort runs separate thread
