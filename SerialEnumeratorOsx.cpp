...
{
    if( !QMetaType::isRegistered( QMetaType::type("PortInformation") ) )
        qRegisterMetaType<PortInformation>("PortInformation");
}
...
{
    QList<PortInformation> infoList;
...
void SerialEnumerator::iterateServicesOSX(io_object_t service, QList<PortInformation> & infoList)
{
...
bool SerialEnumerator::getServiceDetailsOSX( io_object_t service, PortInformation* portInfo )
{
    bool retval = true;
...
void deviceDiscoveredCallbackOSX( void *ctxt, io_iterator_t serialPortIterator )
{
    SerialEnumerator* qese = (SerialEnumerator*)ctxt;
...
/*
  ...
  Create a PortInformation if possible, and emit the signal indicating that we've found it.
*/
void SerialEnumerator::onDeviceDiscoveredOSX( io_object_t service )
{
    PortInformation info;
    info.vendorID = 0;
    info.productID = 0;
...
/*
  ...
  Create a PortInformation if possible, and emit the signal indicating that it's gone.
*/
void SerialEnumerator::onDeviceTerminatedOSX( io_object_t service )
{
    PortInformation info;
    info.vendorID = 0;
    info.productID = 0;
